...
from mmrg.doc2json.s2orc import Paper

from mmrg.doc2json.utils.grobid_util import parse_bib_entry, extract_paper_metadata_from_grobid_xml
from mmrg.doc2json.utils.citation_util import SINGLE_BRACKET_REGEX, BRACKET_REGEX, BRACKET_STYLE_THRESHOLD
from mmrg.doc2json.utils.citation_util import is_expansion_string, _clean_empty_and_duplicate_authors_from_grobid_parse
from mmrg.doc2json.utils.refspan_util import sub_spans_and_update_indices
...
def extract_body_text_from_div(
    ...
    if n_text_tags == 0:
        # Add a blank text unit just so that the section header can be output
        # (e.g., otherwise a top-level section that immediately breaks into
        # subsections won't be captured in s2orc output)
        chunks.append({
            'text': '',
            'cite_spans': [],
            'ref_spans': [],
